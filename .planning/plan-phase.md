...
@~/.gemini/antigravity/get-shit-done/references/revision-loop.md
...
When `TDD_MODE` is `true`, the planner agent is instructed to apply `type: tdd` to eligible tasks using heuristics from `references/tdd.md`. The planner's `<required_reading>` is extended to include `@~/.gemini/antigravity/get-shit-done/references/tdd.md` so gate enforcement rules are available during planning.
...
**If found:**
1. Read template: `~/.gemini/antigravity/get-shit-done/templates/VALIDATION.md`
...
SPIKE_FINDINGS_PATH=$(ls ./.agent/skills/spike-findings-*/SKILL.md 2>/dev/null | head -1)
SKETCH_FINDINGS_PATH=$(ls ./.agent/skills/sketch-findings-*/SKILL.md 2>/dev/null | head -1)
```
...
${TDD_MODE === 'true' ? `
<tdd_mode_active>
**TDD Mode is ENABLED.** Apply TDD heuristics from @~/.gemini/antigravity/get-shit-done/references/tdd.md to all eligible tasks:
...
if [ "$POST_PLANNING_GAPS" = "true" ]; then
  node "$HOME/.gemini/antigravity/get-shit-done/bin/gsd-tools.cjs" gap-analysis --phase-dir "${PHASE_DIR}"
fi
```
