...
<required_reading>
@~/.gemini/antigravity/get-shit-done/references/domain-probes.md
@~/.gemini/antigravity/get-shit-done/references/gate-prompts.md
@~/.gemini/antigravity/get-shit-done/references/universal-anti-patterns.md
</required_reading>
...
if [ -f "$HOME/.gemini/antigravity/get-shit-done/USER-PROFILE.md" ]; then
  ADVISOR_MODE=true
else
  ADVISOR_MODE=false
fi
...
```bash
SPIKE_FINDINGS=$(ls ./.agent/skills/spike-findings-*/SKILL.md 2>/dev/null | head -1)
SKETCH_FINDINGS=$(ls ./.agent/skills/sketch-findings-*/SKILL.md 2>/dev/null | head -1)
...
Read `@~/.gemini/antigravity/get-shit-done/references/scout-codebase.md` — it contains the phase-type→map selection table, single-read rule, no-maps fallback, and `<codebase_context>` output schema. Then execute:
